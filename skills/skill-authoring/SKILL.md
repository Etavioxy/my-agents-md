...
**description：** 描述流程和基础信息，不写原因，触发词仅放明确的，不含模糊词。

**不写触发词的条件**（够任一条即可）：① 只能点名 / 斜杠调用；② 和名字本身已写全，触发词不应和名字语义重复；③ 仅在流程里被别的 skill 按阶段 invoke，不靠自然语言命中；④ 用户说法无法穷举，或同一句话在别的 skill 也成立。

**skill 放：** 已验证的类型/场景清单、关联文档的文件名列表。

**skill 不放：** 未经用户确认的场景。具体项目名、仓库名、路径等指向特定项目的信息。涉及隐私的信息。
