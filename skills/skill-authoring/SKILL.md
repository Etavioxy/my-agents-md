...
- 通用化：优先从原始经验中提炼可复用的通用结论，而非照搬。经过验证的具体发现可以入，同时标注适用前提，原始经验只能写入 Notes 域。
- 可配置占位：避免在可配置字段上硬编码确切数值（如超时时间、序号、数据信息），代以占位符或说明。
- 不保留过时内容：修改规则时，废弃机制属于无效信息，必须直接移除，不保留「已废弃」表述，过时内容仅允许出现在 Notes 域。
...
每个 Skill 的 git 仓库在 SKILL.md 所在文件夹。
